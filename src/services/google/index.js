...
var GoogleSpreadsheet = function(ss_key, auth_id, options) {
        ...
        request(
          ...
          function(err, response, body) {
            if (err) {
              ...
              return cb(
                new Error('HTTP error ' + response.statusCode + ' (' + http.STATUS_CODES[response.statusCode]) +
                  ') - ' +
                  message
              );
  ...
  this.addRow = function(worksheet_id, data, cb) {
    ...
    self.makeFeedRequest(['list', ss_key, worksheet_id], 'POST', data_xml, function(err, data, new_xml) {
      if (err) return cb(err);
      if (typeof new_xml === 'undefined') {
        return cb('New XML undefined');
      }
      var entries_xml = new_xml.match(/<entry[^>]*>([\s\S]*?)<\/entry>/g);
      if (entries_xml === null) {
        return cb('Entries XML undefined');
      }
...
var SpreadsheetCell = function(spreadsheet, worksheet_id, data) {
  ...
  self.save = function(cb) {
    ...
    var edit_id =
      'https://spreadsheets.google.com/feeds/cells/key/worksheetId/private/full/R' + self.row + 'C' + self.col;
    var data_xml =
      '<entry><id>' +
      self.id +
      '</id>' +
